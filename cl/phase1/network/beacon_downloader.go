...
func (f *ForwardBeaconDownloader) RequestMore(ctx context.Context) {
	count := uint64(32)
	respChan := make(chan peerAndBlocks, 5)
	nopeersErrChan := make(chan error, 5)
	log.Warn("begin request more")
	...
	requestFn = func(ctx context.Context, loopCount int) {
		...
		if err != nil {
			...
			if errors.Is(err, peers.PeerRecover) {
				log.Warn("Recover peers available for beacon blocks by range request", "err", err, "peer", peerId, "slot", reqSlot, "reqCount", reqCount)
				nopeersErrChan <- err
				needRetry = false
			}
		...
		log.Warn("recv block len", "size", len(responses))
	...
	case atomicResp, ok := <-respChan:
		if ok {
			cancel()
			log.Warn("forward sync req done", "size", len(atomicResp.blocks))
