			...
			Loop:
				for i := 0; ; {
					...
					case blobs, ok := <-respChan:
						...
						log.Warn("inserted equal check", "ids.len", ids.Len(), "inserted", inserted, "NetworkUnstable", NetworkUnstable(), "nestcall", nestcall)
...
func chainTipSync(ctx context.Context, logger log.Logger, cfg *Cfg, args Args) error {
	...
	ctx, cn := context.WithTimeout(ctx, 600*time.Second)
	defer cn()
