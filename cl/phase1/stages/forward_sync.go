		...
		Loop:
			for i := 0; ; {
				...
				case blobs, ok := <-respChan:
					...
					log.Warn("inserted equal check in forward", "ids.len", ids.Len(), "inserted", inserted, "NetworkUnstable", NetworkUnstable(), "nestcall", nestcall)
					...
					if errverify == nil && inserted == uint64(ids.Len()) {
						...
						if !lastLoop && i > 2 && !nestcall {
							cancel()
							log.Warn("SetNetworkUnstable in forward", "i", i)
							SetNetworkUnstable(true)
							errRet = nil
							if err1 := retryProcess(entity); err1 != nil {
								return err1
