...
//go:embed bootstrap_nodes.yaml
var mainBootNodesCfg embed.FS

func getMainNetNodes() []string {
	data, err := mainBootNodesCfg.ReadFile("bootstrap_nodes.yaml")
	if err != nil {
		log.Fatalf("bootstrap_nodes file not found: %v", err)
		panic(-1)
	}
	var nodeNames []string
	err = yaml.Unmarshal(data, &nodeNames)
	if err != nil {
		log.Fatalf("decode bootstrap_nodes.yaml failed: %v", err)
		panic(-1)
	}
...
var (
	MainnetBootstrapNodes = getMainNetNodes()
